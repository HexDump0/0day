...
# name, source, in-point (s), duration (s), tint
...
def treat(name: str, src: Path, ss: float, t: float, tint: str) -> dict:
    lo, hi = percentiles(src, ss, t)
    span = max(hi - lo, 1.0)
    ...
    dst = DST / f"{name}.mp4"
    subprocess.run(
        ["ffmpeg", "-v", "quiet", "-y", "-ss", str(ss), "-t", str(t),
         "-i", str(src), "-vf", vf, "-an",
...
def main() -> None:
    ...
    out = {}
    for name, srcname, ss, t, tint in CLIPS:
        out[name] = treat(name, RAW / srcname, ss, t, tint)
